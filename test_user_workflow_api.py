#!/usr/bin/env python3
"""
Test the User Workflow API with enhanced image modification
"""
...
def test_session_status(session_id):
    ...
    try:
        response = requests.get(
            f"http://localhost:8000/user-workflow/session/{session_id}/status",
            timeout=30
        )
...
def main():
    """Main test function"""
    print("🚀 Starting User Workflow API Tests")
    print("=" * 60)
    ...
    if session_id and (image_mod_success or direct_endpoints_success):
        print("\n🎉 User workflow API with image modification is working!")
        return True
    else:
